  ...
  app.get('/catalog/part/:partId', function (req, res) {
    ...
    connection.query(query, function (err, rows, fields) {
      if(rows.length){
        ...
        // console.log(rows);
        if(rows[0].allNumbers[0]!==""){
          query = "SELECT distinct p.description, p.comment, p.weight, inventoryManufacturers.fullName as manufacturerFullName, inventoryNumbers.number, p.id, p.price, p.stock, p.ordered, p.link from inventoryNumbers, inventory as p, inventoryManufacturers where inventoryManufacturers.id = inventoryNumbers.manufacturerId and inventoryNumbers.inventoryId = p.id and p.id <> " + req.params.partId + " and inventoryNumbers.number = '" + rows[0].allNumbers[0].number + "' order by inventoryNumbers.main desc";
          // console.log(query);
          var part = rows[0];
          connection.query(query, function (err, rows, fields) {
            if(err) {
